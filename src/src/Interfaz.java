...
public class Interfaz extends javax.swing.JFrame {
    ...
    private void InsertarForma(String forma, int x, int y) {
        ...
        if (forma.equals("Elipse") && y == 5) {
            y = 0;        //Puramente estetico con cordenadas >=0 y <=300 no hay errores
        }
        if (forma.equals("Rectangulo") && y == 5) {
            y = 0;    //Puramente estetico    
        }
        if (forma.equals("Elipse") && y == 295) {
            y = 300;     //Puramente estetico
        }
        if (forma.equals("Rectangulo") && y == 295) {
            y = 300; //Puramente estetico
        }
    ...
    private void RecolectarEntradas() {
        Rectangle2D rectangulo;
        for (int i = 0; i < PanelDibujo.getWidth() - 200; i += 5) {
            for (int j = 0; j < PanelDibujo.getHeight() - 200; j += 5) {
                rectangulo = new Rectangle2D.Double(i, j, 200, 200);
